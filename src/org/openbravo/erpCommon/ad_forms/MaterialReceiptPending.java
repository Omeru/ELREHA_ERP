...
public class MaterialReceiptPending extends HttpSecureAppServlet {
  ...
  private OBError processPurchaseOrder(VariablesSecureApp vars, String strcOrderLineId)
    ...
    try {
      ...
      if (!strcOrderLineId.equals("")) {
        ...
        while (st.hasMoreTokens()) {
          ...
          if (! (dataLine[0].mProductUomId.equals(""))) {
            BigDecimal qty2UOMOrder=new BigDecimal(dataLine[0].quantityorder);
            BigDecimal qtyOrdered=new BigDecimal(strQtyordered);
            BigDecimal qtyOrder=new BigDecimal(dataLine[0].qtyordered);
            
            qty2UOMOrder=qty2UOMOrder.setScale(2, BigDecimal.ROUND_HALF_UP);
            qtyOrdered=qtyOrdered.setScale(2, BigDecimal.ROUND_HALF_UP);
            qtyOrder=qtyOrder.setScale(2, BigDecimal.ROUND_HALF_UP);
            
            qty2ndUOM=qty2UOMOrder.multiply(qtyOrdered.divide(qtyOrder,new MathContext(10)));
            strQqty2ndUOM=qty2ndUOM.toString();
          }
          if ((MaterialReceiptPendingData.isAutomaticActivationConfigured(this, vars.getOrg()).equals("N")) &&
             (SerialNumberData.pdc_getSerialBatchType4product(this, dataLine[0].mProductId).equals("BATCH")))
      ...
      if (strMessageType.equals("Error"))
