...
public class SL_Order_Amt  extends ProductTextHelper  {
      ...
      try {
        if (! strProduct.isEmpty()) {
          printPage(response, vars, strChanged, strQtyOrdered, strPriceActual, strDiscount,
            ...
            strQty, strPriceStd, cancelPriceAd, strOrderUOM, strOrderQTY, strMPricelistId,strMManufacturerID);
        } else {
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strChanged,
      ...
      String strOrderUOM, String strOrderQTY,String strMPricelistId,String strMManufacturerID)
    ...
    if (strChanged.equals("QtyOrdered")) {
      ...
      if (!strOrderUOM.equals("")) {
        BigDecimal a = qtyPurchase.divide(multiplyRate).setScale(stdPrecision, BigDecimal.ROUND_HALF_UP);
    ...
    } else if ((strChanged.equals("inpqtyordered") ||strChanged.equals("inpmManufacturerId")) && !("Y".equals(cancelPriceAd))) { // calculate Actual
        
        priceActual = new BigDecimal(SLOrderProductData.getOffersPrice(this,dataOrder[0].dateordered,
            dataOrder[0].cBpartnerId,strProduct,OrderQTY.compareTo(BigDecimal.ZERO)==0?qtyOrdered.toString():OrderQTY.toString(), dataOrder[0].mPricelistId,strOrderUOM,strMManufacturerID, dataOrder[0].id)).setScale(PricePrecision, BigDecimal.ROUND_HALF_UP);
        if (strChanged.equals("inpmManufacturerId") && Issotrx.equals("N") ){
          resultado.append("new Array(\"inppricelist\", " + SLOrderAmtData.getPricestdAmt(this, strProduct,  dataOrder[0].mPricelistId, strOrderUOM, strMManufacturerID,dataOrder[0].cBpartnerId) + "),");
          resultado.append("new Array(\"inppricestd\", " + SLOrderAmtData.getPricestdAmt(this, strProduct,  dataOrder[0].mPricelistId, strOrderUOM, strMManufacturerID,dataOrder[0].cBpartnerId) + "),");
        }
        if (priceActual.scale() > PricePrecision)
          priceActual = priceActual.setScale(PricePrecision, BigDecimal.ROUND_HALF_UP);
       ...
       // Standard- bzw. Mindest-Bestellmenge aus Einkauf (m_product_po) beruecksichtigen, wenn hinterlegt     
        if (strChanged.equals("inpqtyordered")||strChanged.equals("inpmManufacturerId")) {
          ...
          if (!qtyPurchase.equals(OrderQTY.compareTo(BigDecimal.ZERO)==0?qtyOrdered:OrderQTY)) {
    		...
    		if(!SLOrderAmtData.mrp_elr_getPriceAd(this, strProduct, dataOrder[0].cBpartnerId)[0].equals("") && SLOrderAmtData.mrp_elr_getPriceAd(this, strProduct, dataOrder[0].cBpartnerId)[0]!=null)
    	    {
    ...
    if (strChanged.equals("inpmManufacturerId")||strChanged.equals("inpmProductUomId")) {   
      String strPositionText = this.getDocumentText(strProduct, dataOrder[0].cBpartnerId, Issotrx,dataOrder[0].adOrgId ,vars.getLanguage(),strOrderUOM,strMManufacturerID);
