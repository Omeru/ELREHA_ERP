 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_Invoice" +
      "        SET C_Invoice_ID = (?) , C_Order_ID = (?) , DocumentNo = (?) , DateInvoiced = TO_DATE(?) , DateOrdered = TO_DATE(?) , DateAcct = TO_DATE(?) , DocStatus = (?) , C_DocType_ID = (?) , Description = (?) , DueAmt = TO_NUMBER(?) , DaysTillDue = TO_NUMBER(?) , C_PaymentTerm_ID = (?) , PaymentRule = (?) , C_Currency_ID = (?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , C_Project_ID = (?) , IsPaid = (?) , IsGrossInvoice = (?) , TotalLines = TO_NUMBER(?) , GrandTotal = TO_NUMBER(?) , TotalPaid = TO_NUMBER(?) , GenerateTo = (?) , User1_ID = (?) , C_Withholding_ID = (?) , SalesRep_ID = (?) , AD_Client_ID = (?) , User2_ID = (?) , Processing = (?) , C_Campaign_ID = (?) , AD_User_ID = (?) , Posted = (?) , IsPrinted = (?) , IsTaxIncluded = (?) , C_DocTypeTarget_ID = (?) , Taxdate = TO_DATE(?) , IsDiscountPrinted = (?) , AD_OrgTrx_ID = (?) , DatePrinted = TO_DATE(?) , CopyFrom = (?) , UpdatePaymentMonitor = (?) , C_Activity_ID = (?) , DocAction = (?) , M_PriceList_ID = (?) , LastCalculatedOnDate = TO_DATE(?) , Processed = (?) , ChargeAmt = TO_NUMBER(?) , IsSelfService = (?) , OutstandingAmt = TO_NUMBER(?) , IsActive = (?) , AD_Org_ID = (?) , POReference = (?) , Withholdingamount = TO_NUMBER(?) , IsSOTrx = (?) , C_Charge_ID = (?) , CreateFrom = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_Invoice.C_Invoice_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (C_Invoice_ID, C_Order_ID, DocumentNo, DateInvoiced, DateOrdered, DateAcct, DocStatus, C_DocType_ID, Description, DueAmt, DaysTillDue, C_PaymentTerm_ID, PaymentRule, C_Currency_ID, C_BPartner_ID, C_BPartner_Location_ID, C_Project_ID, IsPaid, IsGrossInvoice, TotalLines, GrandTotal, TotalPaid, GenerateTo, User1_ID, C_Withholding_ID, SalesRep_ID, AD_Client_ID, User2_ID, Processing, C_Campaign_ID, AD_User_ID, Posted, IsPrinted, IsTaxIncluded, C_DocTypeTarget_ID, Taxdate, IsDiscountPrinted, AD_OrgTrx_ID, DatePrinted, CopyFrom, UpdatePaymentMonitor, C_Activity_ID, DocAction, M_PriceList_ID, LastCalculatedOnDate, Processed, ChargeAmt, IsSelfService, OutstandingAmt, IsActive, AD_Org_ID, POReference, Withholdingamount, IsSOTrx, C_Charge_ID, CreateFrom, created, createdby, updated, updatedBy)" +
