 ...
 */
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (Ref_OrderLine_ID, IsDescription, AD_Client_ID, AD_Org_ID, M_Offer_ID, C_Order_ID, C_BPartner_ID, C_BPartner_Location_ID, DateOrdered, Line, Linetaxamt, DatePromised, Isgrossprice, DirectShip, M_Warehouse_ID, IsOneTimePosition, A_Asset_ID, M_Product_ID, Issummaryitem, M_AttributeSetInstance_ID, S_ResourceAssignment_ID, Description, QuantityOrder, M_Product_Uom_Id, QtyOrdered, C_UOM_ID, QtyDelivered, QtyReserved, QtyInvoiced, Invoicedamt, Ignoreresidue, M_Shipper_ID, PriceActual, C_Currency_ID, PriceStd, CANCELPRICEAD, PriceList, Discount, FreightAmt, C_Tax_ID, C_Project_ID, C_Projectphase_ID, C_Projecttask_ID, LineNetAmt, Linegrossamt, DateInvoiced, C_Charge_ID, DateDelivered, C_OrderLine_ID, PriceLimit, ChargeAmt, IsActive, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), (?), (?), (?), TO_DATE(?), TO_NUMBER(?), TO_NUMBER(?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_DATE(?), (?), TO_DATE(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), now(), ?, now(), ?)";
