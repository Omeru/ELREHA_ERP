...
static Logger log4j = Logger.getLogger(TranslationData.class);
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("created"))
      ...
      return updatedbyr;
    else if (fieldName.equalsIgnoreCase("c_uom_trl_id") || fieldName.equals("cUomTrlId"))
      return cUomTrlId;
...
Select for edit
 */
  public static TranslationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cUomId, String key, String adUserClient, String adOrgClient)    throws ServletException {
...
Select for edit
 */
  public static TranslationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cUomId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT to_char(C_UOM_Trl.Created, ?) as created, " +
      "        (SELECT NAME FROM AD_USER u WHERE AD_USER_ID = C_UOM_Trl.CreatedBy) as CreatedByR, " +
      "        to_char(C_UOM_Trl.Updated, ?) as updated, " +
      "        to_char(C_UOM_Trl.Updated, 'YYYYMMDDHH24MISS') as Updated_Time_Stamp,  " +
      "        C_UOM_Trl.UpdatedBy, " +
      "        (SELECT NAME FROM AD_USER u WHERE AD_USER_ID = C_UOM_Trl.UpdatedBy) as UpdatedByR," +
      "        C_UOM_Trl.C_UOM_Trl_ID, " +
      "C_UOM_Trl.AD_Client_ID, " +
      "(CASE WHEN C_UOM_Trl.AD_Client_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Client_IDR, " +
      "C_UOM_Trl.AD_Org_ID, " +
      "(CASE WHEN C_UOM_Trl.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_UOM_Trl.C_UOM_ID, " +
      "(CASE WHEN C_UOM_Trl.C_UOM_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS C_UOM_IDR, " +
      "C_UOM_Trl.AD_Language, " +
      "(CASE WHEN C_UOM_Trl.AD_Language IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS AD_LanguageR, " +
      "COALESCE(C_UOM_Trl.IsActive, 'N') AS IsActive, " +
      "COALESCE(C_UOM_Trl.IsTranslated, 'N') AS IsTranslated, " +
      "C_UOM_Trl.UOMSymbol, " +
      "C_UOM_Trl.Name, " +
      "C_UOM_Trl.Description, " +
      "        ? AS LANGUAGE " +
      "        FROM C_UOM_Trl left join (select AD_Client_ID, Name from AD_Client) table1 on (C_UOM_Trl.AD_Client_ID = table1.AD_Client_ID) left join (select AD_Org_ID, Name from AD_Org) table2 on (C_UOM_Trl.AD_Org_ID = table2.AD_Org_ID) left join (select C_UOM_ID, Name from C_UOM) table3 on (C_UOM_Trl.C_UOM_ID = table3.C_UOM_ID) left join (select C_UOM_ID,AD_Language, Name from C_UOM_TRL) tableTRL3 on (table3.C_UOM_ID = tableTRL3.C_UOM_ID and tableTRL3.AD_Language = ?)  left join (select AD_Language, Name from AD_Language) table4 on (C_UOM_Trl.AD_Language =  table4.AD_Language)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
    strSql = strSql + ((cUomId==null || cUomId.equals(""))?"":"  AND C_UOM_Trl.C_UOM_ID = ?  ");
    strSql = strSql + 
      "        AND C_UOM_Trl.C_UOM_Trl_ID = ? " +
      "        AND C_UOM_Trl.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
      ") " +
      "           AND C_UOM_Trl.AD_Org_ID IN (";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectTranslationData.cUomTrlId = UtilSql.getValue(result, "c_uom_trl_id");
        ...
        objectTranslationData.cUomId = UtilSql.getValue(result, "c_uom_id");
        objectTranslationData.cUomIdr = UtilSql.getValue(result, "c_uom_idr");
        ...
        objectTranslationData.uomsymbol = UtilSql.getValue(result, "uomsymbol");
...
Create a registry
 */
  public static TranslationData[] set(String cUomId, String uomsymbol, String createdby, String createdbyr, String description, String adClientId, String adLanguage, String adOrgId, String istranslated, String updatedby, String updatedbyr, String cUomTrlId, String isactive, String name)    throws ServletException {
    ...
    objectTranslationData[0].cUomTrlId = cUomTrlId;
    ...
    objectTranslationData[0].cUomId = cUomId;
    objectTranslationData[0].cUomIdr = "";
    ...
    objectTranslationData[0].uomsymbol = uomsymbol;
 ...
 */
  public static String selectDef3964_0(ConnectionProvider connectionProvider, String CreatedByR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
...
Select for auxiliar field
 */
  public static String selectDef3966_1(ConnectionProvider connectionProvider, String UpdatedByR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
 ...
 */
  public static String selectParentID(ConnectionProvider connectionProvider, String key)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT C_UOM_Trl.C_UOM_ID AS NAME" +
      "        FROM C_UOM_Trl" +
      "        WHERE C_UOM_Trl.C_UOM_Trl_ID = ?";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_UOM_Trl" +
      "        SET C_UOM_Trl_ID = (?) , AD_Client_ID = (?) , AD_Org_ID = (?) , C_UOM_ID = (?) , AD_Language = (?) , IsActive = (?) , IsTranslated = (?) , UOMSymbol = (?) , Name = (?) , Description = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_UOM_Trl.C_UOM_Trl_ID = ? " +
      "                 AND C_UOM_Trl.C_UOM_ID = ? " +
      "        AND C_UOM_Trl.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
      ") " +
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, cUomTrlId);
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (C_UOM_Trl_ID, AD_Client_ID, AD_Org_ID, C_UOM_ID, AD_Language, IsActive, IsTranslated, UOMSymbol, Name, Description, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), (?), (?), (?), (?), (?), now(), ?, now(), ?)";
