...
static Logger log4j = Logger.getLogger(TranslationData.class);
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("created"))
      ...
      return updatedbyr;
    else if (fieldName.equalsIgnoreCase("c_doctype_trl_id") || fieldName.equals("cDoctypeTrlId"))
...
Select for edit
 */
  public static TranslationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cDoctypeId, String key, String adUserClient, String adOrgClient)    throws ServletException {
    return selectEdit(connectionProvider, dateTimeFormat, paramLanguage, cDoctypeId, key, adUserClient, adOrgClient, 0, 0);
  }

/**
Select for edit
 */
  public static TranslationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cDoctypeId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT to_char(C_DocType_Trl.Created, ?) as created, " +
      "        (SELECT NAME FROM AD_USER u WHERE AD_USER_ID = C_DocType_Trl.CreatedBy) as CreatedByR, " +
      "        to_char(C_DocType_Trl.Updated, ?) as updated, " +
      "        to_char(C_DocType_Trl.Updated, 'YYYYMMDDHH24MISS') as Updated_Time_Stamp,  " +
      "        C_DocType_Trl.UpdatedBy, " +
      "        (SELECT NAME FROM AD_USER u WHERE AD_USER_ID = C_DocType_Trl.UpdatedBy) as UpdatedByR," +
      "        C_DocType_Trl.C_DocType_Trl_ID, " +
      "C_DocType_Trl.AD_Client_ID, " +
      "(CASE WHEN C_DocType_Trl.AD_Client_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS AD_Client_IDR, " +
      "C_DocType_Trl.AD_Org_ID, " +
      "(CASE WHEN C_DocType_Trl.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS AD_Org_IDR, " +
      "C_DocType_Trl.C_DocType_ID, " +
      "(CASE WHEN C_DocType_Trl.C_DocType_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR((CASE WHEN tableTRL3.Name IS NULL THEN TO_CHAR(table3.Name) ELSE TO_CHAR(tableTRL3.Name) END)), ''))),'') ) END) AS C_DocType_IDR, " +
      "C_DocType_Trl.AD_Language, " +
      "(CASE WHEN C_DocType_Trl.AD_Language IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table4.Name), ''))),'') ) END) AS AD_LanguageR, " +
      "COALESCE(C_DocType_Trl.IsActive, 'N') AS IsActive, " +
      "COALESCE(C_DocType_Trl.IsTranslated, 'N') AS IsTranslated, " +
      "C_DocType_Trl.Name, " +
      "C_DocType_Trl.PrintName, " +
      "C_DocType_Trl.DocumentNote, " +
      "        ? AS LANGUAGE " +
      "        FROM C_DocType_Trl left join (select AD_Client_ID, Name from AD_Client) table1 on (C_DocType_Trl.AD_Client_ID = table1.AD_Client_ID) left join (select AD_Org_ID, Name from AD_Org) table2 on (C_DocType_Trl.AD_Org_ID = table2.AD_Org_ID) left join (select C_DocType_ID, Name from C_DocType) table3 on (C_DocType_Trl.C_DocType_ID = table3.C_DocType_ID) left join (select C_DocType_ID,AD_Language, Name from C_DocType_TRL) tableTRL3 on (table3.C_DocType_ID = tableTRL3.C_DocType_ID and tableTRL3.AD_Language = ?)  left join (select AD_Language, Name from AD_Language) table4 on (C_DocType_Trl.AD_Language =  table4.AD_Language)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
    strSql = strSql + ((cDoctypeId==null || cDoctypeId.equals(""))?"":"  AND C_DocType_Trl.C_DocType_ID = ?  ");
    strSql = strSql + 
      "        AND C_DocType_Trl.C_DocType_Trl_ID = ? " +
      "        AND C_DocType_Trl.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
      ") " +
      "           AND C_DocType_Trl.AD_Org_ID IN (";
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectTranslationData.cDoctypeTrlId = UtilSql.getValue(result, "c_doctype_trl_id");
        ...
        objectTranslationData.cDoctypeId = UtilSql.getValue(result, "c_doctype_id");
        objectTranslationData.cDoctypeIdr = UtilSql.getValue(result, "c_doctype_idr");
...
Create a registry
 */
  public static TranslationData[] set(String cDoctypeId, String createdby, String createdbyr, String isactive, String adLanguage, String adOrgId, String name, String adClientId, String printname, String documentnote, String cDoctypeTrlId, String updatedby, String updatedbyr, String istranslated)    throws ServletException {
    ...
    objectTranslationData[0].cDoctypeTrlId = cDoctypeTrlId;
    ...
    objectTranslationData[0].cDoctypeId = cDoctypeId;
    objectTranslationData[0].cDoctypeIdr = "";
 ...
 */
  public static String selectDef3131_0(ConnectionProvider connectionProvider, String CreatedByR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
...
Select for auxiliar field
 */
  public static String selectDef3133_1(ConnectionProvider connectionProvider, String UpdatedByR)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
 ...
 */
  public static String selectParentID(ConnectionProvider connectionProvider, String key)    throws ServletException {
    String strSql = "";
    strSql = strSql + 
      "        SELECT C_DocType_Trl.C_DocType_ID AS NAME" +
      "        FROM C_DocType_Trl" +
      "        WHERE C_DocType_Trl.C_DocType_Trl_ID = ?";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET C_DocType_Trl_ID = (?) , AD_Client_ID = (?) , AD_Org_ID = (?) , C_DocType_ID = (?) , AD_Language = (?) , IsActive = (?) , IsTranslated = (?) , Name = (?) , PrintName = (?) , DocumentNote = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_DocType_Trl.C_DocType_Trl_ID = ? " +
      "                 AND C_DocType_Trl.C_DocType_ID = ? " +
      "        AND C_DocType_Trl.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
      ") " +
    ...
    st = connectionProvider.getPreparedStatement(conn, strSql);
      iParameter++; UtilSql.setValue(st, iParameter, 12, null, cDoctypeTrlId);
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (C_DocType_Trl_ID, AD_Client_ID, AD_Org_ID, C_DocType_ID, AD_Language, IsActive, IsTranslated, Name, PrintName, DocumentNote, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), (?), (?), (?), (?), (?), now(), ?, now(), ?)";
