 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET C_Order_ID = (?) , DateOrdered = TO_DATE(?) , DocumentNo = (?) , POReference = (?) , C_Invoice_ID = (?) , Description = (?) , Internalnote = (?) , C_DocTypeTarget_ID = (?) , DateInvoiced = TO_DATE(?) , DateAcct = TO_DATE(?) , performanceperiodstart = TO_DATE(?) , performanceperiodend = TO_DATE(?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , AD_User_ID = (?) , M_PriceList_ID = (?) , C_Currency_ID = (?) , SalesRep_ID = (?) , C_Charge_ID = (?) , ChargeAmt = TO_NUMBER(?) , PaymentRule = (?) , C_PaymentTerm_ID = (?) , Deliveryrule = (?) , C_Project_ID = (?) , C_Projecttask_ID = (?) , Totalpaid = TO_NUMBER(?) , OutstandingAmt = TO_NUMBER(?) , Discountamt = TO_NUMBER(?) , Writeoffamt = TO_NUMBER(?) , Transactiondate = TO_DATE(?) , Schedtransactiondate = TO_DATE(?) , Ispaid = (?) , TotalLines = TO_NUMBER(?) , GrandTotal = TO_NUMBER(?) , C_DocType_ID = (?) , DocStatus = (?) , Posted = (?) , IsSOTrx = (?) , Isgrossinvoice = (?) , C_Activity_ID = (?) , Processed = (?) , IsSelfService = (?) , Taxdate = TO_DATE(?) , C_Campaign_ID = (?) , IsTaxIncluded = (?) , CopyFrom = (?) , Btnreinvoiceprojectexpenses = (?) , IsDiscountPrinted = (?) , IsPrinted = (?) , C_Withholding_ID = (?) , UpdatePaymentMonitor = (?) , DocAction = (?) , DatePrinted = TO_DATE(?) , User2_ID = (?) , Withholdingamount = TO_NUMBER(?) , AD_Client_ID = (?) , Processing = (?) , IsPaymentshedulesummary = (?) , A_Asset_ID = (?) , LastCalculatedOnDate = TO_DATE(?) , DaysTillDue = TO_NUMBER(?) , C_Projectphase_ID = (?) , DueAmt = TO_NUMBER(?) , CreateFrom = (?) , IsActive = (?) , C_Refinvoicetoorder_View_ID = (?) , AD_OrgTrx_ID = (?) , GenerateTo = (?) , AD_Org_ID = (?) , User1_ID = (?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (C_Order_ID, DateOrdered, DocumentNo, POReference, C_Invoice_ID, Description, Internalnote, C_DocTypeTarget_ID, DateInvoiced, DateAcct, performanceperiodstart, performanceperiodend, C_BPartner_ID, C_BPartner_Location_ID, AD_User_ID, M_PriceList_ID, C_Currency_ID, SalesRep_ID, C_Charge_ID, ChargeAmt, PaymentRule, C_PaymentTerm_ID, Deliveryrule, C_Project_ID, C_Projecttask_ID, Totalpaid, OutstandingAmt, Discountamt, Writeoffamt, Transactiondate, Schedtransactiondate, Ispaid, TotalLines, GrandTotal, C_DocType_ID, DocStatus, Posted, User1_ID, IsSOTrx, Isgrossinvoice, C_Activity_ID, Processed, IsSelfService, Taxdate, C_Campaign_ID, IsTaxIncluded, CopyFrom, Btnreinvoiceprojectexpenses, IsDiscountPrinted, IsPrinted, C_Withholding_ID, UpdatePaymentMonitor, DocAction, DatePrinted, User2_ID, Withholdingamount, AD_Client_ID, Processing, IsPaymentshedulesummary, A_Asset_ID, LastCalculatedOnDate, DaysTillDue, C_Projectphase_ID, DueAmt, CreateFrom, IsActive, C_Refinvoicetoorder_View_ID, AD_OrgTrx_ID, GenerateTo, AD_Org_ID, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), TO_DATE(?), (?), (?), (?), (?), (?), (?), TO_DATE(?), TO_DATE(?), TO_DATE(?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_DATE(?), TO_DATE(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_DATE(?), (?), TO_NUMBER(?), (?), (?), (?), (?), TO_DATE(?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), (?), (?), (?), (?), (?), now(), ?, now(), ?)";
