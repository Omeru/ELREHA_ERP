 ...
 */
  public static BusinessPartnerData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String mOfferId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Offer_BPartner.M_Offer_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Name), ''))),'') ) END) AS M_Offer_IDR, " +
      "M_Offer_BPartner.C_BPartner_ID, " +
      "(CASE WHEN M_Offer_BPartner.C_BPartner_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table2.Name), ''))),'') ) END) AS C_BPartner_IDR, " +
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_Offer_BPartner left join (select M_Offer_ID, Name from M_Offer) table1 on (M_Offer_BPartner.M_Offer_ID = table1.M_Offer_ID) left join (select C_BPartner_ID, Name from C_BPartner) table2 on (M_Offer_BPartner.C_BPartner_ID = table2.C_BPartner_ID)" +
      "        WHERE 2=2 " +
      "        AND 1=1 ";
...
Create a registry
 */
  public static BusinessPartnerData[] set(String mOfferId, String cBpartnerId, String cBpartnerIdr, String createdby, String createdbyr, String adClientId, String adOrgId, String updatedby, String updatedbyr, String isactive, String mOfferBpartnerId)    throws ServletException {
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET M_Offer_ID = (?) , C_BPartner_ID = (?) , M_Offer_Bpartner_ID = (?) , AD_Client_ID = (?) , AD_Org_ID = (?) , IsActive = (?) , updated = now(), updatedby = ? " +
      "        WHERE M_Offer_BPartner.M_Offer_Bpartner_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (M_Offer_ID, C_BPartner_ID, M_Offer_Bpartner_ID, AD_Client_ID, AD_Org_ID, IsActive, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), (?), now(), ?, now(), ?)";
