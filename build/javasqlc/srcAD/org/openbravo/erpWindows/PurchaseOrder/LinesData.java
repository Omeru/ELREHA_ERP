 ...
 */
  public static LinesData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String cOrderId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "C_OrderLine.M_Manufacturer_ID, " +
      "(CASE WHEN C_OrderLine.M_Manufacturer_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table11.Name), ''))),'') ) END) AS M_Manufacturer_IDR, " +
      ...
      "(CASE WHEN C_OrderLine.A_Asset_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table15.Name), ''))),'') ) END) AS A_Asset_IDR, " +
      "C_OrderLine.C_Project_ID, " +
      "(CASE WHEN C_OrderLine.C_Project_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table16.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table16.Name), ''))),'') ) END) AS C_Project_IDR, " +
      "C_OrderLine.C_Projecttask_ID, " +
      "(CASE WHEN C_OrderLine.C_Projecttask_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table17.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table18.Value), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table18.Name), ''))),'') ) END) AS C_Projecttask_IDR, " +
      "C_OrderLine.Orderlineselfjoin, " +
      "(CASE WHEN C_OrderLine.Orderlineselfjoin IS NULL THEN '' ELSE  ( COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table20.DocumentNo), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table20.Name), ''))),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(table20.DateOrdered, 'DD-MM-YYYY')),'')  || ' - ' || COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table20.GrandTotal), ''))),'') ) END) AS OrderlineselfjoinR, " +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectLinesData.mManufacturerIdr = UtilSql.getValue(result, "m_manufacturer_idr");
 ...
 */
  public static LinesData[] set(String cOrderId, String mProductUomId, String mWarehouseId, String datepromised, String aAssetId, String isgrossprice, String qtyinvoiced, String adClientId, String securityqty, String calcfastestdeliverydate, String invoicedamt, String createdby, String createdbyr, String quantityorder, String discount, String linenetamt, String mOfferId, String pricelimit, String linegrossamt, String updatedby, String updatedbyr, String qtydelivered, String cOrderIdr, String directship, String qtyordered, String cProjecttaskId, String mManufacturerId, String refOrderlineId, String cProjectId, String cProjectIdr, String ignoreresiduedate, String mAttributesetinstanceId, String mAttributesetinstanceIdr, String cBpartnerLocationId, String isactive, String cancelpricead, String priceactual, String isdescription, String deliverycomplete, String mShipperId, String cOrderlineId, String cChargeId, String isonetimeposition, String dateinvoiced, String cBpartnerId, String cBpartnerIdr, String cProjectphaseId, String cUomId, String scheddeliverydate, String mProductId, String mProductIdr, String adOrgId, String freightamt, String chargeamt, String orderlineselfjoin, String linetaxamt, String qtyreserved, String dateordered, String sResourceassignmentId, String pricestd, String calloffqty, String line, String cCurrencyId, String datedelivered, String pricelist, String issummaryitem, String cTaxId, String description, String ignoreresidue)    throws ServletException {
    ...
    objectLinesData[0].mManufacturerIdr = "";
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_OrderLine" +
      "        SET AD_Client_ID = (?) , AD_Org_ID = (?) , C_Order_ID = (?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , DateOrdered = TO_DATE(?) , DatePromised = TO_DATE(?) , Scheddeliverydate = TO_DATE(?) , Line = TO_NUMBER(?) , M_Product_ID = (?) , Calcfastestdeliverydate = (?) , M_AttributeSetInstance_ID = (?) , C_Charge_ID = (?) , Description = (?) , QuantityOrder = TO_NUMBER(?) , M_Product_Uom_Id = (?) , QtyOrdered = TO_NUMBER(?) , C_UOM_ID = (?) , Securityqty = TO_NUMBER(?) , QtyDelivered = TO_NUMBER(?) , M_Manufacturer_ID = (?) , QtyInvoiced = TO_NUMBER(?) , Invoicedamt = TO_NUMBER(?) , Ignoreresidue = (?) , Deliverycomplete = (?) , M_Shipper_ID = (?) , PriceActual = TO_NUMBER(?) , C_Currency_ID = (?) , PriceStd = TO_NUMBER(?) , CANCELPRICEAD = (?) , PriceList = TO_NUMBER(?) , Discount = TO_NUMBER(?) , FreightAmt = TO_NUMBER(?) , C_Tax_ID = (?) , A_Asset_ID = (?) , C_Project_ID = (?) , C_Projecttask_ID = (?) , Orderlineselfjoin = (?) , LineNetAmt = TO_NUMBER(?) , Linegrossamt = TO_NUMBER(?) , Calloffqty = TO_NUMBER(?) , Ref_OrderLine_ID = (?) , ChargeAmt = TO_NUMBER(?) , QtyReserved = TO_NUMBER(?) , DateInvoiced = TO_DATE(?) , M_Offer_ID = (?) , DateDelivered = TO_DATE(?) , IsDescription = (?) , C_Projectphase_ID = (?) , IsActive = (?) , C_OrderLine_ID = (?) , Ignoreresiduedate = TO_DATE(?) , IsOneTimePosition = (?) , M_Warehouse_ID = (?) , Issummaryitem = (?) , PriceLimit = TO_NUMBER(?) , Linetaxamt = TO_NUMBER(?) , DirectShip = (?) , Isgrossprice = (?) , S_ResourceAssignment_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_OrderLine.C_OrderLine_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Client_ID, AD_Org_ID, C_Order_ID, C_BPartner_ID, C_BPartner_Location_ID, DateOrdered, DatePromised, Scheddeliverydate, Line, M_Product_ID, Calcfastestdeliverydate, M_AttributeSetInstance_ID, C_Charge_ID, Description, QuantityOrder, M_Product_Uom_Id, QtyOrdered, C_UOM_ID, Securityqty, QtyDelivered, M_Manufacturer_ID, QtyInvoiced, Invoicedamt, Ignoreresidue, Deliverycomplete, M_Shipper_ID, PriceActual, C_Currency_ID, PriceStd, CANCELPRICEAD, PriceList, Discount, FreightAmt, C_Tax_ID, A_Asset_ID, C_Project_ID, C_Projecttask_ID, Orderlineselfjoin, LineNetAmt, Linegrossamt, Calloffqty, Ref_OrderLine_ID, ChargeAmt, QtyReserved, DateInvoiced, M_Offer_ID, DateDelivered, IsDescription, C_Projectphase_ID, IsActive, C_OrderLine_ID, Ignoreresiduedate, IsOneTimePosition, M_Warehouse_ID, Issummaryitem, PriceLimit, Linetaxamt, DirectShip, Isgrossprice, S_ResourceAssignment_ID, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), TO_DATE(?), TO_DATE(?), TO_DATE(?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_DATE(?), (?), TO_DATE(?), (?), (?), (?), (?), TO_DATE(?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), now(), ?, now(), ?)";
