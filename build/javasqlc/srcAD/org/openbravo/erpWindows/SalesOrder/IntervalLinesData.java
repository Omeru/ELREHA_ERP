 ...
 */
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (Ref_OrderLine_ID, IsDescription, AD_Client_ID, M_Offer_ID, AD_Org_ID, C_Subscriptioninterval_View_ID, C_BPartner_ID, C_BPartner_Location_ID, DateOrdered, Linetaxamt, Line, Isgrossprice, DatePromised, C_Projectphase_ID, Scheddeliverydate, DirectShip, A_Asset_ID, M_Warehouse_ID, Isonetimeposition, M_Product_ID, Issummaryitem, M_AttributeSetInstance_ID, S_ResourceAssignment_ID, Description, QuantityOrder, C_Subscriptionintervallines_View_ID, M_Product_Uom_Id, QtyOrdered, C_UOM_ID, QtyDelivered, QtyReserved, QtyInvoiced, Invoicedamt, Ignoreresidue, M_Shipper_ID, PriceActual, C_Currency_ID, PriceStd, CANCELPRICEAD, PriceList, Discount, FreightAmt, C_Tax_ID, C_Project_ID, C_Projecttask_ID, LineNetAmt, Linegrossamt, DateDelivered, C_Charge_ID, ChargeAmt, IsActive, PriceLimit, DateInvoiced, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), (?), (?), (?), TO_DATE(?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_DATE(?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_DATE(?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_DATE(?), now(), ?, now(), ?)";
