 ...
 */
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, IsActive, zssm_productionorder_v_id, Name, Ended, C_ProjectTask_ID, Value, SeqNo, Started, StartDate, Enddate, Description, Triggerreason, Qty, Qtyproduced, M_Product_ID, Assembly, Receiving_Locator, Issuing_Locator, Servcostplan, Machinecostplan, Materialcostplan, Plannedcost, Iscommitceiling, Planmaterial, Outsourcing, PriceActual, Actualcost, BeginTask, Canceltask, Createbom, Machinecost, Help, Expenses, Invoicedamt, Zssm_Productionplan_Task_ID, Datecontract, C_Orderline_ID, Endtask, C_Task_ID, Zssm_Workstep_V_ID, Getmaterialfromstock, Gotopurchasing, Iscomplete, Materialcost, Servcost, Percentdone, Taskbegun, Returnmaterialtostock, Ismaterialdisposed, Indirectcost, Indirectcostplan, Istaskcancelled, AD_Client_ID, Percentrejects, Committedamt, Qtyleft, Schedulestatus, Unplanmaterial, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), TO_DATE(?), (?), (?), TO_NUMBER(?), TO_DATE(?), TO_DATE(?), TO_DATE(?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), TO_DATE(?), (?), (?), (?), (?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), now(), ?, now(), ?)";
