 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE A_Asset" +
      "        SET AD_Client_ID = (?) , AD_Org_ID = (?) , Assettype = (?) , IsActive = (?) , AD_User_ID = (?) , Value = (?) , Name = (?) , Help = (?) , Description = (?) , Lot = (?) , VersionNo = (?) , SNR_Masterdata_ID = (?) , C_Project_ID = (?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , LocationComment = (?) , AssetDisposalDate = TO_DATE(?) , GuaranteeDate = TO_DATE(?) , UseLifeYears = TO_NUMBER(?) , UseLifeMonths = TO_NUMBER(?) , AssetServiceDate = TO_DATE(?) , LifeUseUnits = TO_NUMBER(?) , C_Location_ID = (?) , Amortizationvalueamt = TO_NUMBER(?) , Depreciatedvalue = TO_NUMBER(?) , IsInPosession = (?) , Profit = TO_NUMBER(?) , IsDepreciated = (?) , A_Asset_Group_ID = (?) , Processed = (?) , AssetDepreciationDate = TO_DATE(?) , Amortizationtype = (?) , IsDisposed = (?) , Assetschedule = (?) , Acctvalueamt = TO_NUMBER(?) , Iscostcenter = (?) , Amortizationcalctype = (?) , IsOwned = (?) , IsFullyDepreciated = (?) , Issummary = (?) , A_Asset_ID = (?) , UseUnits = TO_NUMBER(?) , DocumentNo = (?) , Depreciatedpreviousamt = TO_NUMBER(?) , SerNo = (?) , Datepurchased = TO_DATE(?) , Datecancelled = TO_DATE(?) , Amortizationstartdate = TO_DATE(?) , Amortizationenddate = TO_DATE(?) , Annualamortizationpercentage = TO_NUMBER(?) , C_Currency_ID = (?) , AssetValueAmt = TO_NUMBER(?) , Residualassetvalueamt = TO_NUMBER(?) , Depreciatedplan = TO_NUMBER(?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO A_Asset " +
      "        (AD_Client_ID, AD_Org_ID, Assettype, IsActive, AD_User_ID, Value, Name, Help, Description, Lot, VersionNo, SNR_Masterdata_ID, C_Project_ID, C_BPartner_ID, C_BPartner_Location_ID, LocationComment, AssetDisposalDate, GuaranteeDate, UseLifeYears, UseLifeMonths, AssetServiceDate, LifeUseUnits, C_Location_ID, Amortizationvalueamt, Depreciatedvalue, IsInPosession, Profit, IsDepreciated, A_Asset_Group_ID, Processed, AssetDepreciationDate, Amortizationtype, IsDisposed, Assetschedule, Acctvalueamt, Iscostcenter, Amortizationcalctype, IsOwned, IsFullyDepreciated, Issummary, A_Asset_ID, UseUnits, DocumentNo, Depreciatedpreviousamt, SerNo, Datepurchased, Datecancelled, Amortizationstartdate, Amortizationenddate, Annualamortizationpercentage, C_Currency_ID, AssetValueAmt, Residualassetvalueamt, Depreciatedplan, created, createdby, updated, updatedBy)" +
