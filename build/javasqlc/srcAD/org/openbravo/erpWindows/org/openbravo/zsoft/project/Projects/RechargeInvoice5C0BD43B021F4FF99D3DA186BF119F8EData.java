 ...
 */
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (Zspm_Recharge_View_ID, C_Project_ID, C_Projecttask_ID, AD_Client_ID, AD_Org_ID, Qtyinvoiced, M_Product_ID, Isactive, C_Order_ID, C_Orderline_ID, C_Bpartner_ID, Dateordered, Salesrep_ID, C_Invoice_ID, C_Invoiceline_ID, Description, Dateinvoiced, Linenetamt, Ispaid, Transactiondate, Recharge_Qtyinvoiced, Recharge_Product_ID, Recharge_Invoice_ID, Reinvoicedby_ID, Totallines, Recharge_Dateinvoiced, Recharge_Bpartner_ID, Grandtotal, Recharge_Linenetamt, Totalpaid, Recharge_Order_ID, Isrecharged, Norecharge, Norechargecomment, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), TO_NUMBER(?), (?), (?), (?), (?), (?), TO_DATE(?), (?), (?), (?), (?), TO_DATE(?), TO_NUMBER(?), (?), TO_DATE(?), TO_NUMBER(?), (?), (?), (?), TO_NUMBER(?), TO_DATE(?), (?), TO_NUMBER(?), TO_NUMBER(?), TO_NUMBER(?), (?), (?), (?), (?), now(), ?, now(), ?)";
