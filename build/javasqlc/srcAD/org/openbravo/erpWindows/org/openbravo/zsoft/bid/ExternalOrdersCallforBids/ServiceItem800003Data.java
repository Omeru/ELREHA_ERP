 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET CommittedQty = TO_NUMBER(?) , C_ProjectLine_ID = (?) , IsPrinted = (?) , InvoicedQty = TO_NUMBER(?) , InvoicedAmt = TO_NUMBER(?) , M_Product_Category_ID = (?) , C_Order_ID = (?) , C_OrderPO_ID = (?) , C_ProjectIssue_ID = (?) , DoPricing = (?) , CommittedAmt = TO_NUMBER(?) , Processed = (?) , AD_Client_ID = (?) , AD_Org_ID = (?) , C_Project_ID = (?) , Line = TO_NUMBER(?) , IsActive = (?) , Description = (?) , M_Product_ID = (?) , Product_Value = (?) , Product_Name = (?) , Product_Description = (?) , PlannedQty = TO_NUMBER(?) , C_Tax_ID = (?) , PlannedPrice = TO_NUMBER(?) , Plannedpoprice = TO_NUMBER(?) , PlannedAmt = TO_NUMBER(?) , PlannedMarginAmt = TO_NUMBER(?) , updated = now(), updatedby = ? " +
      "        WHERE C_ProjectLine.C_ProjectLine_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (CommittedQty, C_ProjectLine_ID, IsPrinted, InvoicedQty, InvoicedAmt, M_Product_Category_ID, C_Order_ID, C_OrderPO_ID, C_ProjectIssue_ID, DoPricing, CommittedAmt, Processed, AD_Client_ID, AD_Org_ID, C_Project_ID, Line, IsActive, Description, M_Product_ID, Product_Value, Product_Name, Product_Description, PlannedQty, C_Tax_ID, PlannedPrice, Plannedpoprice, PlannedAmt, PlannedMarginAmt, created, createdby, updated, updatedBy)" +
