 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_Order" +
      "        SET Grandtotalonetime = TO_NUMBER(?) , Lostproposalfixedreason = (?) , Subscriptionchangedate = TO_DATE(?) , C_BPartner_ID = (?) , DatePrinted = TO_DATE(?) , Estpropability = (?) , Delivery_Location_ID = (?) , Weekly_Day = (?) , ChargeAmt = TO_NUMBER(?) , GrandTotal = TO_NUMBER(?) , IsActive = (?) , Weight_Uom = (?) , bpzipcode = (?) , TotalLines = TO_NUMBER(?) , A_Asset_ID = (?) , Quarterly_Month = (?) , DatePromised = TO_DATE(?) , C_PaymentTerm_ID = (?) , Deliverynotes = (?) , DeliveryViaRule = (?) , Transactionreference = (?) , FreightCostRule = (?) , Processed = (?) , TotalPaid = TO_NUMBER(?) , C_Order_ID = (?) , Enddate = TO_DATE(?) , C_Currency_ID = (?) , Weight = TO_NUMBER(?) , SalesRep_ID = (?) , TransactionDate = TO_DATE(?) , InvoiceRule = (?) , M_Shipper_ID = (?) , SchedDeliveryDate = TO_DATE(?) , Internalnote = (?) , DateOrdered = TO_DATE(?) , Generateproject = (?) , IsTaxIncluded = (?) , CompleteOrderValue = TO_NUMBER(?) , Totallinesonetime = TO_NUMBER(?) , C_Incoterms_ID = (?) , Proposalstatus = (?) , C_Projecttask_ID = (?) , Invoicefrequence = (?) , DocStatus = (?) , IsPrinted = (?) , C_BPartner_Location_ID = (?) , C_DocType_ID = (?) , BillTo_ID = (?) , Firstschedinvoicedate = TO_DATE(?) , IsInvoiced = (?) , SchedTransactionDate = TO_DATE(?) , Isrecharge = (?) , C_DocTypeTarget_ID = (?) , DateAcct = TO_DATE(?) , IsCompletelyInvoiced = (?) , C_Project_ID = (?) , Isinvoiceafterfirstcycle = (?) , Ispaid = (?) , PaymentRule = (?) , FreightAmt = TO_NUMBER(?) , PriorityRule = (?) , Lostproposalreason = (?) , IsDelivered = (?) , Yearly_Month = (?) , Posted = (?) , Contractdate = TO_DATE(?) , BtnCopyTemplate = (?) , C_Bidproject_ID = (?) , IsSOTrx = (?) , Incotermsdescription = (?) , Name = (?) , M_Warehouse_ID = (?) , M_PriceList_ID = (?) , Closeproject = (?) , Description = (?) , M_Product_ID = (?) , DropShip_BPartner_ID = (?) , DocAction = (?) , DocumentNo = (?) , Monthly_Day = TO_NUMBER(?) , Qty = TO_NUMBER(?) , IsSelected = (?) , DropShip_Location_ID = (?) , DeliveryRule = (?) , AD_User_ID = (?) , IsDiscountPrinted = (?) , InvoicedAmt = TO_NUMBER(?) , AD_Client_ID = (?) , AD_Org_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_Order.C_Order_ID = ? " +
      "        AND C_Order.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO C_Order " +
      "        (Grandtotalonetime, Lostproposalfixedreason, Subscriptionchangedate, C_BPartner_ID, DatePrinted, Estpropability, Delivery_Location_ID, Weekly_Day, ChargeAmt, GrandTotal, IsActive, Weight_Uom, bpzipcode, TotalLines, A_Asset_ID, Quarterly_Month, DatePromised, C_PaymentTerm_ID, Deliverynotes, DeliveryViaRule, Transactionreference, FreightCostRule, Processed, TotalPaid, C_Order_ID, Enddate, C_Currency_ID, Weight, SalesRep_ID, TransactionDate, InvoiceRule, M_Shipper_ID, SchedDeliveryDate, Internalnote, DateOrdered, Generateproject, IsTaxIncluded, CompleteOrderValue, Totallinesonetime, C_Incoterms_ID, Proposalstatus, C_Projecttask_ID, Invoicefrequence, DocStatus, IsPrinted, C_BPartner_Location_ID, C_DocType_ID, BillTo_ID, Firstschedinvoicedate, IsInvoiced, SchedTransactionDate, Isrecharge, C_DocTypeTarget_ID, DateAcct, IsCompletelyInvoiced, C_Project_ID, Isinvoiceafterfirstcycle, Ispaid, PaymentRule, FreightAmt, PriorityRule, Lostproposalreason, IsDelivered, Yearly_Month, Posted, Contractdate, BtnCopyTemplate, C_Bidproject_ID, IsSOTrx, Incotermsdescription, Name, M_Warehouse_ID, M_PriceList_ID, Closeproject, Description, M_Product_ID, DropShip_BPartner_ID, DocAction, DocumentNo, Monthly_Day, Qty, IsSelected, DropShip_Location_ID, DeliveryRule, AD_User_ID, IsDiscountPrinted, InvoicedAmt, AD_Client_ID, AD_Org_ID, created, createdby, updated, updatedBy)" +
