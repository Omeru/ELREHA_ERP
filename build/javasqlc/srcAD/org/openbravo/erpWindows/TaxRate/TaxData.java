 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_Tax" +
      "        SET SOPOType = (?) , IsTaxExempt = (?) , AD_Client_ID = (?) , AD_Org_ID = (?) , Name = (?) , Description = (?) , Line = TO_NUMBER(?) , Taxhint = (?) , Cascade = (?) , IsActive = (?) , IsWithholdingTax = (?) , ValidFrom = TO_DATE(?) , Rate = TO_NUMBER(?) , Deducpercent = TO_NUMBER(?) , Reversecharge = (?) , Originalrate = TO_NUMBER(?) , Adduid2fact = (?) , IsTaxUndeductable = (?) , C_Country_ID = (?) , IsNoVAT = (?) , Datevkeyvst = (?) , Datevkeyust = (?) , IsNoTaxable = (?) , IsSummary = (?) , C_Region_ID = (?) , To_Country_ID = (?) , C_Tax_ID = (?) , To_Region_ID = (?) , IsDefault = (?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO C_Tax " +
      "        (IsTaxExempt, SOPOType, AD_Client_ID, AD_Org_ID, Name, Description, Line, Taxhint, Cascade, IsActive, IsWithholdingTax, ValidFrom, Rate, Deducpercent, Reversecharge, Originalrate, Adduid2fact, IsTaxUndeductable, C_Country_ID, IsNoVAT, Datevkeyvst, Datevkeyust, IsNoTaxable, IsSummary, C_Region_ID, To_Country_ID, C_Tax_ID, To_Region_ID, IsDefault, created, createdby, updated, updatedBy)" +
