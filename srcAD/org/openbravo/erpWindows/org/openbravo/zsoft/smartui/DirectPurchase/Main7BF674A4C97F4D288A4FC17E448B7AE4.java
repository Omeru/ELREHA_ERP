...
/*    } else if (vars.commandIn("DELETE_RELATION")) {
      ...
      String strZssi_Directpurchase_ID = vars.getRequiredInStringParameter("inpzssiDirectpurchaseId");
      String message = deleteRelation(vars, strZssi_Directpurchase_ID);
      if (!message.equals("")) {
...
/*
  String deleteRelation(VariablesSecureApp vars, String strZssi_Directpurchase_ID) throws IOException, ServletException {
    log4j.debug("Deleting records");
    Connection conn = this.getTransactionConnection();
    try {
      if (strZssi_Directpurchase_ID.startsWith("(")) strZssi_Directpurchase_ID = strZssi_Directpurchase_ID.substring(1, strZssi_Directpurchase_ID.length()-1);
      if (!strZssi_Directpurchase_ID.equals("")) {
        strZssi_Directpurchase_ID = Replace.replace(strZssi_Directpurchase_ID, "'", "");
        StringTokenizer st = new StringTokenizer(strZssi_Directpurchase_ID, ",", false);
        while (st.hasMoreTokens()) {
          String strKey = st.nextToken();
          if (Main7BF674A4C97F4D288A4FC17E448B7AE4Data.deleteTransactional(conn, this, strKey)==0) {
...
String strParamDateofsale_f = vars.getSessionValue(tabId + "|paramDateofsale_f");
    ...
    if (!(strParamDateofsale.equals("") && strParamM_Product_ID.equals("") && strParamC_Bpartner_ID.equals("") && strParamDateofsale_f.equals(""))) hasSearchCondition=true;
    String strOffset = "0";
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = SequenceIdData.getUUID();
          ...
          CrudOperations.UpdateCustomFields(tabId, data.zssiDirectpurchaseId, vars, this);
        } catch(Exception ex) {
