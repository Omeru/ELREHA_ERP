...
*/
  private OverviewCalendarEventsData getEditVariables(Connection con, VariablesSecureApp vars) throws IOException,ServletException {
   ...
   private OverviewCalendarEventsData[] getRelationData(OverviewCalendarEventsData[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].cBpartnerId = FormatUtilities.truncate(data[i].cBpartnerId, 32);        data[i].cBpartneremployeeeventId = FormatUtilities.truncate(data[i].cBpartneremployeeeventId, 32);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 32);        data[i].cCalendareventId = FormatUtilities.truncate(data[i].cCalendareventId, 32);        data[i].note = FormatUtilities.truncate(data[i].note, 50);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 32);}
    }
    return data;
  }
