...
*/
  private EmailConfigurationData getEditVariables(Connection con, VariablesSecureApp vars, String strPAD_Client_ID) throws IOException,ServletException {
   ...
   private EmailConfigurationData[] getRelationData(EmailConfigurationData[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 12);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 12);        data[i].smtpserver = FormatUtilities.truncate(data[i].smtpserver, 20);        data[i].smtpserveraccount = FormatUtilities.truncate(data[i].smtpserveraccount, 20);        data[i].smtpserverpassword = FormatUtilities.truncate(data[i].smtpserverpassword, 20);        data[i].smtpserversenderaddress = FormatUtilities.truncate(data[i].smtpserversenderaddress, 30);        data[i].centralfromemail = FormatUtilities.truncate(data[i].centralfromemail, 40);        data[i].cPocConfigurationId = FormatUtilities.truncate(data[i].cPocConfigurationId, 12);}
    }
    return data;
  }
